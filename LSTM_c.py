...
class Lstm(object):
    ...
    def lstm_model(self):
        ...
        prob_sube = pd.read_csv('LSproblemli_sube_komsu_oranlari_prtfy_v2.csv', sep=';')
        ##Problemli Subeler icin bu verileri kullanacaksın. BURADAN DEVAM ET.
        prob = pd.read_csv('problem_sube_LSTM.csv',sep=';',encoding='ISO:8859-1')
        ...
        if self.sube in list_sorunlu_sube:
            ...
            df2= pd.read_csv('LSproblemli_sube_komsu_oranlari_prtfy_v2.csv', sep=';')#Sorunlu subelerin düzenlenmis dönem mevduat hedefleri.)
        ...
        #Veriyi lstm katmanına hazırlamak icin reshapeledik. 3 Boyutlu matris haline getirildi.
        trainX = np.reshape(trainX, (trainX.shape[0], 1, trainX.shape[1]))
        testX = np.reshape(testX, (testX.shape[0], 1, testX.shape[1]))
        forc = np.reshape(forc, (forc.shape[0], 1, forc.shape[1]))
...
try:
    f = open("LSTMSonuc.txt", 'w')  # write in text mode
    f.write("LSTM SUBE MODEL METRIKLERI\n")
    for sube in subeler:
        f.write("####### {} #######\n".format(sube))
...
problem_sube.to_csv('problem_sube_LSTM.csv', index=False, sep=';',encoding='ISO:8859-1')"""
###################################################################
...
prob_sube.to_csv('LSproblemli_sube_komsu_oranlari_prtfy_v2.csv', sep=';')#Sorunlu subelerin düzenlenmis dönem mevduat hedefleri."""
#######################################################################